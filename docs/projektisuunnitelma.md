# Projektisuunnitelma

- [Projektisuunnitelma](#projektisuunnitelma)
  - [Projektin tiedot](#projektin-tiedot)
  - [Projektin sisältö](#projektin-sisältö)
    - [Kuvaus](#kuvaus)
    - [Projektiryhmä](#projektiryhmä)
    - [Tavoite](#tavoite)
  - [Aikataulu](#aikataulu)
  - [Raportointi, kommunikointi ja dokumentointi](#raportointi-kommunikointi-ja-dokumentointi)
    - [Raportointi](#raportointi)
    - [Tuntikirjaukset](#tuntikirjaukset)
    - [Palaverikäytänteet](#palaverikäytänteet)
    - [Kommunikointitavat](#kommunikointitavat)
    - [Dokumentointi](#dokumentointi)
    - [Dokumenttien hallinta](#dokumenttien-hallinta)
  - [Riskien hallinta](#riskien-hallinta)

## Projektin tiedot

/<Oscards>

Tekijät:
Joel Hänninen
Tuomas Huhtala
Nathalie Penttinen
Patrick Uimonen

## Projektin sisältö

### Kuvaus

_IMDB-pohjainen, oscarin voittaneiden näyttelijöiden sekä ohjaajien esittelysovellus.
Tarjolla on kortteja näyttelijöistä ja ohjaajista sisältäen heidän perustietonsa.
Näyttelijää/ohjaajaa klikkaamalla kortti/kortit kääntyy, ja valikko tarjoilee heidän top 5 titleään, perustiedot ja saatavuuden suoratoistopalveluissa._

### Projektiryhmä

_Ketä projektiin kuuluu ja mitkä ovat henkilöiden päävastuut._

| Nimi               | Vastuurooli                             |
| ------------------ | --------------------------------------- |
| Tuomas Huhtala     | Git-päällikkö/ Fronttikoodaus           |
| Patrick Uimonen    | Dokumentti-päällikkö/Fronttikoodaus     |
| Nathalie Penttinen | Tehtävä-päällikkö/UI-UX                 |
| Joel Hänninen      | Julkaisu/Laadunhallinta-päällikkö/UI-UX |

### Tavoite

_Projektin tavoiteltuna lopputuloksena olisi saada elokuvakulttuurista maistiaista, tietoa näyttelijöistä, elokuvista ja ohjaajista sekä mahdollisesti myös elokuvasuosituksia._

## Aikataulu

_Kirjaa tähän lukuun aikataulu. Millä tavoin projekti viedään läpi. Voitte itse päättää mikä taso on teidän projektillenne paras._

| Tapahtuma        | Päivämäärä | Muut tiedot/osanottajat                  |
| ---------------- | ---------- | ---------------------------------------- |
| Projekti alkaa   | 20.04.2026 | Ohjelmistoprojekti 1 alkaa               |
| Idean valinta    | 23.04.2026 | Valitaan tuotantoon menevä projekti-idea |
| ............     | pvm        | Kaikki muut tapahtumat listattuna        |
| Projekti päättyy | 16.05.2026 | Ohjelmistoprojekti 1 päättyy             |

## Raportointi, kommunikointi ja dokumentointi

### Raportointi

_Päivisin järjestetään sekä aamuisin että päivän loputtua lyhyt palaveri, jossa kirjataan tarvittavat ongelmat tms. ylös_

### Tuntikirjaukset

_Tuntikirjaukset hoituvat valmiiksi annettuihin taulukoihin, ja niitä päivitetään/muutetaan joka päivä._

### Palaverikäytänteet

_Projektin viikkopalaveri pidetään perjantai-aamupäivisin senhetkisessä työskentelytilassa. Mahdollisiin ongelmiin päästään viimeistään tällöin tarttumaan ja korjaustoimenpiteistä sovitaan._

### Kommunikointitavat

_Projektin jäsenet ovat paikalla ja läsnä työskennellessä. Tämän lisäksi Teams ja Whatsapp toimivat kommunikaatiovälineinä._

### Dokumentointi

_Etenemisvaiheessa otetaan kuvia ja mahdollisesti kirjoitetaan kehitysvaiheista kriittistä tietoa talteen. Myös viikkoraportit ovat tärkeitä._

### Dokumenttien hallinta

_Dokumentit sisältävät versionumerot_

## Riskien hallinta

_Kuvaa tässä kuinka projekti aikoo hallinnoida ja seurata projektiriskejä._

Esimerkiksi näin.

| Riskit                                                                     | Todennäköisyys | Impakti     | Minimointi                                   |
| -------------------------------------------------------------------------- | -------------- | ----------- | -------------------------------------------- |
| Työntekijät eivät ole motivoituneita ja eivät saavu paikalle               | Pieni          | Suuri       | Pidetään työpaikalla kahvia ja pullaa.       |
| Joku ei ymmärrä jotain asiaa itse                                          | Suht pieni     | Suuri       | Kysyy missä kohtaan tahansa muun tiimin apua |
| Alkaa uuvuttaa ja istuminen sattuu, jonka vuoksi keskittyminenkin hankalaa | Suuri          | Kohtalainen | Pidetään riittävästi niitä taukoja!          |
